  ...
  async start(user: AuthUser, input: StartWorkflowInput) {
    ...
    if (!supabase) {
      ...
      if (process.status !== "active") {
        throw new WorkflowExecutionError(
          "PROCESS_NOT_ACTIVE",
          "Workflows can only be started from active SOPs.",
        );
      }

      const version = processDemoStore.getVersion(process.currentVersionId);
      if (!version) {
        throw new WorkflowExecutionError("NOT_FOUND", "Process version not found.");
      }

      const steps = workflowDemoStore.resolveProcessSteps(version.id);
      const assigneeMap = new Map(
        (input.assignees ?? []).map((item) => [item.stepId, item.userId]),
      );
      const now = new Date().toISOString();
      const instance = workflowDemoStore.createInstance({
        tenantId: user.tenantId,
      ...
      const createdTasks = workflowDemoStore.createTasksFromSteps(
        user.tenantId,
        instance.id,
        steps,
        assigneeMap,
      ...
      workflowDemoStore.appendAudit({
        ...
        actorId: user.id,
        actorName: user.email,
        action: `Started workflow "${instance.title}"`,
        occurredAt: now,
      });

      return {
        ...this.toWorkflowDetail(instance, createdTasks),
        tasks: createdTasks.map((task) => this.toTaskRecord(task)),
      };
    }

    const { data: process, error: processError } = await supabase
      .from("processes")
      .select("id, process_code, name, status, current_version_id")
      .eq("tenant_id", user.tenantId)
    ...
    if (!steps || steps.length === 0) {
      throw new WorkflowExecutionError(
        "INVALID_STATE",
        "Process version has no executable steps.",
      );
    }
    ...
    await supabase.from("workflow_instances").insert({
      ...
      process_version_id: process.current_version_id as string,
      title: input.title.trim(),
    ...
    const taskRows = steps.map((step, index) => ({
      ...
      process_step_id: step.id as string,
      step_number: step.step_number as number,
      title: step.title as string,
      description: (step.description as string | null) ?? null,
      step_type: step.step_type === "approval" ? "approval" : "manual",
      status: index === 0 ? "in_progress" : "pending",
      assigned_to: assigneeMap.get(step.id as string) ?? null,
      evidence_required: Boolean(step.evidence_required),
      started_at: index === 0 ? now : null,
    }));

    await supabase.from("workflow_tasks").insert(taskRows);

    const detail = await this.get(user, instanceId);
    return detail!;
  }
